...
test('Verify Promotion Enrollment page displays all required elements')
...
test('Verify User can navigate back from Promotion Enrollment page')
.tags(TAG.REGRESSION)
.id('PROTEST-6806')
.it(async ({ currentPromotionsPage, promotionEnrollmentPage }) => {
let enrollmentPage: ReturnType<typeof promotionEnrollmentPage>;
...
test('Verify User can successfully enroll in a promotion')
.tags(TAG.REGRESSION)
.id('PROTEST-6807')
...
test('Verify Promotion Enrollment page displays correctly in Spanish')
.tags(TAG.REGRESSION)
.id('PROTEST-6808')
