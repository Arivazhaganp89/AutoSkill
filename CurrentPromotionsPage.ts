...
return this.page.getByTestId('current-promotion-box');
}
...
.getByRole('link', { name: promotionName, exact: true })
.locator('..')
.locator('..')
.locator('..');
}
...
await Verifications.verifyVisible(
this.getAmountToNextReward(promotionName),
`${promotionName} amount to next reward`,
);
}
/**
...
await Verifications.verifyVisible(
this.getQualifyingPurchases(promotionName),
`${promotionName} qualifying purchases`,
);
}
/**
...
await Verifications.verifyVisible(
this.getPromotionProgress(promotionName),
`${promotionName} promotion progress`,
);
}
/**
...
*/
async verifyAmountToNextReward(
promotionName: string,
expectedAmount: string,
): Promise<void> {
await Verifications.verifyContainsText(
this.getAmountToNextReward(promotionName),
expectedAmount,
`${promotionName} amount to next reward`,
);
...
*/
async verifyQualifyingPurchases(
promotionName: string,
...
await Verifications.verifyVisible(
this.getEndingSoonStatus(promotionName),
`${promotionName} Ending Soon status`,
);
}
}
